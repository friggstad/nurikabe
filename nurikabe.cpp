...
struct UnionFind {
  ...
  int cnt(int i, int j) { return cnt(pack(i, j)); }
...
int dfs_expand(int r, int c, int rep, const vvi& g, map<int, set<int>>& adj, set<int>& seen) {
  ...
  seen.insert(p);
  // cout << "Trying: " << r << ' ' << c << endl;
  if (g[r][c] == BLACK) return 0;
  if (adj[p].size() >= 2) return 0;
  if (adj[p].size() == 1 && *adj[p].begin() != rep) return 0;
  // cout << "Success!" << endl;
...
bool consistent(const vvi& g) {
  ...
  // CHECK: numbered regions have ok size (not too big yet)
  ...
  for (int i = 0; i < R; ++i)
    for (int j = 0; j < C; ++j)
      if (g[i][j] >= 1) {
        ...
        // if (g[i][j] == 3) cout << uf.cnt(rep) << endl;
  ...
  // REPLACE WITH BFS THAT TERMINATES EARLY IF LARGE ENOUGH
  for (int i = 0; i < R; ++i)
    for (int j = 0; j < C; ++j)
      if (g[i][j] >= 1) {
  ...
  // TODO:
  // Ensure all dotted regions without numbers can reach a numbered region with appropriate size
...
int iterate(vvi& g, int pi, int pj, int max_depth, bool report = true) {
  ...
  for (int depth = 1; depth <= max_depth; ++depth) {
    ...
    for (const pii& cell : order[pi][pj]) {
      ...
      if (dot_ret == 0) {
        // if . lead to contradiction, set it to #
        g[i][j] = BLACK;
        progress = true;
      }
      else {
    ...
    if (progress) depth = 0;
    // else if (report) cout << "Increasing depth to " << depth+1 << endl;
  }
